...
void initLPIT(const uint8_t channel, const uint32_t frequency, const isr_t handler, const uint32_t priority) {
  ...
  LPIT0->TMR[channel].TVAL = LPIT_CLK_FREQ / frequency;    /* channel timer start value */  
  
  /* 46.4.1.6.2 Module Interrupt Enable Register */
  LPIT0->MIER |= (0b1 << (LPIT_MIER_TIE0_SHIFT + channel));    /* Timer Interrupt Enabled for Channel */
  
  /* 46.4.1.9.3 Timer Control Register */  
  LPIT0->TMR[channel].TCTRL &= ~LPIT_TMR_TCTRL_T_EN(0b1); /* T_EN : Timer channel is disabled to set registers*/
